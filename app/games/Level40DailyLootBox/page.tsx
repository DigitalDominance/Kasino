...
// =============================================================================
// LEVEL 40 DAILY LOOT BOX GAME COMPONENT & CONTENT
// =============================================================================
...
// ---------------------------------------------------------
// Main Page Component for Level 40 Daily Loot Box Game
// ---------------------------------------------------------
export default function Level40DailyLootBoxGamePage() {
...
function DailyLootBoxContent() {
  ...
  // REQUIRED_LEVEL for Level 40 loot box is set to 40.
  const REQUIRED_LEVEL = 40;

  // On page load, check localStorage for a stored daily loot box timestamp for Level 40.
  useEffect(() => {
    const storedTimestamp = localStorage.getItem("dailyLootBoxTimestamp_Level40");
    if (storedTimestamp) {
  ...
  const handleOpenLootBox = async () => {
    ...
    try {
      ...
      if (startRes.data.success) {
        ...
        localStorage.setItem("dailyLootBoxTimestamp_Level40", Date.now().toString());
      } else {
      ...
      if (error.response && error.response.status === 403) {
        ...
        console.error("Error starting Level 40 Daily Loot Box game:", error);
  ...
  const handleGameEnd = async (item: any) => {
    ...
    if (gameId) {
      try {
        ...
        console.error("Error ending Level 40 Daily Loot Box game on backend:", error);
      }
    }
  };
...
function DailyLootBoxGame({ isPlaying, onGameEnd }: { isPlaying: boolean; onGameEnd: (item: any) => void; }) {
  ...
  useEffect(() => {
    if (isPlaying && !spinTriggered.current) {
      ...
      let winItem;
      if (r < 0.9999) {
        // 99.99% chance to win a common reward (0.35 KAS)
