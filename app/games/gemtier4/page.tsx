...
// =============================================================================
// GEM CRATE TIER 4 GAME COMPONENT & CONTENT
// =============================================================================

const tier4CrateItems = [
  { id: 1, name: "Lucky Gem", tier: "gem-common", reward: 7.5, image: "/GemCommon1.webp" },
  { id: 2, name: "Shiny Stone", tier: "gem-common", reward: 7.5, image: "/GemCommon2.webp" },
  { id: 3, name: "Sparkling Rock", tier: "gem-common", reward: 7.5, image: "/GemCommon3.webp" },
  { id: 4, name: "Golden Nugget", tier: "gem-common", reward: 7.5, image: "/GemCommon4.webp" },
  { id: 5, name: "Mystic Orb", tier: "gem-rare", reward: 100, image: "/GemCommon5.webp" },
  { id: 6, name: "Enchanted Crystal", tier: "gem-rare", reward: 100, image: "/GemCommon6.webp" },
  { id: 7, name: "Ruby Shard", tier: "gem-rare", reward: 100, image: "/GemCommon7.webp" },
  { id: 8, name: "Diamond Prize", tier: "gem-ultra-rare", reward: 500, image: "/GemUltraRare.webp" },
  { id: 9, name: "Diamond Ghost", tier: "gem-legendary", reward: 1000, image: "/GemLegendary.webp" },
];
...
// Main Page Component for Gem Crate Tier 4 Game
export default function GemCrateTier4GamePage() {
  return <GemCrateTier4Content />;
}

function GemCrateTier4Content() {
  ...
  // Start the Tier 4 crate game on the backend using the gemcrate API
                ...
                <div className="relative w-full max-w-[600px] h-72 mx-auto flex items-center justify-center">
                  ...
                  {!isPlaying && (
                    <div className="absolute inset-0 z-30">
                      {/* Pregame overlay: 4 images (1 common, 1 rare, 1 ultra rare, 1 legendary) */}
                      <motion.div
...
function GemCrateGame({ isPlaying, onGameEnd }: { isPlaying: boolean; onGameEnd: (item: any) => void; }) {
  ...
  useEffect(() => {
    if (isPlaying && !spinTriggered.current) {
      ...
      // Generate a random reel of 40 items from tier4CrateItems
      const randomReel = Array.from({ length: 40 }, () => tier4CrateItems[Math.floor(Math.random() * tier4CrateItems.length)]);
      ...
      // Legendary: 0.1% chance (r < 0.001)
      // Ultra Rare: 1% chance (r < 0.011)
      // Rare: 12.5% chance (r < 0.136)
      // Otherwise common.
      const r = Math.random();
      let winItem;
      if (r < 0.001) {
        winItem = tier4CrateItems.find(item => item.tier === "gem-legendary");
      } else if (r < 0.011) {
        winItem = tier4CrateItems.find(item => item.tier === "gem-ultra-rare");
      } else if (r < 0.136) {
        const rareRewards = tier4CrateItems.filter(item => item.tier === "gem-rare");
        winItem = rareRewards[Math.floor(Math.random() * rareRewards.length)];
      } else {
        const commonRewards = tier4CrateItems.filter(item => item.tier === "gem-common");
              ...
              <p className="text-xl font-bold text-red-100">
                ...
                <span className="text-base text-red-200">
                  {winningItem.tier === "gem-ultra-rare"
                    ...
                    ? "Rare"
                    : winningItem.tier === "gem-legendary"
                    ? "Legendary"
                    : winningItem.tier.replace("gem-", "")}
                </span>
              </p>
              <p className="text-lg text-red-50 mt-2">
