...
// =============================================================================
// GEM CRATE TIER 2 GAME COMPONENT & CONTENT
// =============================================================================
...
const tier2CrateItems = [
  ...
  { id: 8, name: "Diamond Prize", tier: "gem-ultra-rare", reward: 100, image: "/GemUltraRare.webp" },
];
...
// Main Page Component for Gem Crate Tier 2 Game
export default function GemCrateTier2GamePage() {
  return <GemCrateTier2Content />;
}

function GemCrateTier2Content() {
  ...
  // Start the Tier 2 crate game on the backend using the gemcrate API
  const handleOpenCrate = async () => {
    ...
    try {
      ...
      const walletAddress = accounts[0];
      // Pass both the walletAddress and the gems being spent (100 for Tier 2)
    ...
    <div className={`${montserrat.className} min-h-screen bg-black text-white flex flex-col`}>
      <div className="flex-grow p-6">
            ...
            <Card className="bg-green-900/50 border border-green-500 backdrop-blur-sm overflow-hidden">
              <div className="p-6 flex flex-col h-full items-center">
                <div className="flex justify-between items-center w-full mb-4">
                  <h2 className="text-2xl font-bold text-green-300">Gem Crate Tier 2</h2>
        ...
        <Card className="bg-green-900/50 border border-green-500 backdrop-blur-sm p-4 mb-6">
          <h3 className="text-xl font-bold text-green-300 mb-4 text-center">Gem Crate Tier 2 Rewards</h3>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
            {tier2CrateItems.map((item) => {
        ...
        <Card className="w-full bg-green-900/50 border border-green-500 backdrop-blur-sm p-4 flex flex-col items-center text-center">
          ...
          <p className="text-lg text-green-200">Cost per play: 100 Gems</p>
        </Card>
      </div>
      <SiteFooter />
      <AnimatePresence>
...
function GemCrateGame({ isPlaying, onGameEnd }: { isPlaying: boolean; onGameEnd: (item: any) => void; }) {
  ...
  useEffect(() => {
    if (isPlaying && !spinTriggered.current) {
      ...
      // Generate a random reel of 40 items from tier2CrateItems
      const randomReel = Array.from({ length: 40 }, () => tier2CrateItems[Math.floor(Math.random() * tier2CrateItems.length)]);
      ...
      let winItem;
      if (r < 0.0001) {
        winItem = tier2CrateItems.find(item => item.tier === "gem-ultra-rare");
      } else if (r < 0.125) {
        const rareRewards = tier2CrateItems.filter(item => item.tier === "gem-rare");
        winItem = rareRewards[Math.floor(Math.random() * rareRewards.length)];
      } else {
        const commonRewards = tier2CrateItems.filter(item => item.tier === "gem-common");
