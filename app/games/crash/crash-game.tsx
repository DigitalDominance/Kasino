...
export function CrashGame({ isPlaying, onGameEnd, betAmount, onCashoutSuccess, onManualCashout }: CrashGameProps) {
  ...
  const calculateCrashPoint = (): number => {
    ...
    const houseEdge = 0.9 // 10% house edge

    // Ensure minimum crash point is 1.01x and apply house edge
    const baseMultiplier = Math.max(1.01, Math.floor((100 * houseEdge) / (random * 0.99 + 0.01)) / 100)
    const finalCrashPoint = Number(baseMultiplier.toFixed(2))
  ...
  const startGameLoop = (crashPointValue: number) => {
    ...
    const updateGame = (timestamp: number) => {
      ...
      // Calculate new multiplier with smoother progression
      const newMultiplier = Math.min(crashPointValue, 1 + (Math.exp(0.00006 * elapsed) - 1))

      console.log("Current multiplier:", newMultiplier.toFixed(2))
      setCurrentMultiplier(newMultiplier)

      if (timeSinceLastUpdate >= 50) {
        // Update chart every 50ms
        setChartData((prevData) => [...prevData, { x: elapsed / 1000, y: newMultiplier }])
        ...
        {showCashoutPopup && (
          ...
          >
            <div className="bg-[#49EACB]/10 border border-[#49EACB]/20 rounded-lg p-8 max-w-md w-full text-center">
              ...
              <div className="text-3xl font-bold text-[#49EACB]">Won {cashoutAmount.toFixed(8)} KAS</div>
              <button
