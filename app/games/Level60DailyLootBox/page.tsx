...
// =============================================================================
// LEVEL 60 DAILY LOOT BOX GAME COMPONENT & CONTENT
// =============================================================================
...
// ---------------------------------------------------------
// Daily Loot Items Distribution for Level 60 Daily Loot Box
// ---------------------------------------------------------
export const dailyLootItems = [
  { id: 1, name: "Stack Of Coins", tier: "daily-common", reward: 0.6, image: "/DailyCommon1.webp" },
  { id: 2, name: "Gem", tier: "daily-common", reward: 0.6, image: "/DailyCommon2.webp" },
  { id: 3, name: "Money Bag", tier: "daily-common", reward: 0.6, image: "/DailyCommon3.webp" },
  { id: 4, name: "Money Slots", tier: "daily-common", reward: 0.6, image: "/DailyCommon4.webp" },
  { id: 5, name: "Gift of Coins", tier: "daily-common", reward: 0.6, image: "/DailyCommon5.webp" },
  { id: 6, name: "Gem Slots", tier: "daily-common", reward: 0.6, image: "/DailyCommon6.webp" },
  { id: 7, name: "Green Workbag", tier: "daily-common", reward: 0.6, image: "/DailyCommon7.webp" },
  { id: 8, name: "Kasino Chip", tier: "daily-ultra-rare", reward: 1400, image: "/DailyUltraRare.webp" },
];
...
// ---------------------------------------------------------
// Main Page Component for Level 60 Daily Loot Box Game
// ---------------------------------------------------------
export default function Level60DailyLootBoxGamePage() {
...
function DailyLootBoxContent() {
  ...
  // REQUIRED_LEVEL for Level 60 loot box is set to 60.
  const REQUIRED_LEVEL = 60;

  // On page load, check localStorage for a stored daily loot box timestamp for Level 60.
  useEffect(() => {
    const storedTimestamp = localStorage.getItem("dailyLootBoxTimestamp_Level60");
    if (storedTimestamp) {
  ...
  const handleOpenLootBox = async () => {
    ...
    try {
      ...
      if (startRes.data.success) {
        ...
        localStorage.setItem("dailyLootBoxTimestamp_Level60", Date.now().toString());
      } else {
      ...
      if (error.response && error.response.status === 403) {
        ...
        console.error("Error starting Level 60 Daily Loot Box game:", error);
  ...
  const handleGameEnd = async (item: any) => {
    ...
    if (gameId) {
      try {
        ...
        console.error("Error ending Level 60 Daily Loot Box game on backend:", error);
      }
    }
  };
    ...
    <div className={`${montserrat.className} min-h-screen bg-black text-white flex flex-col`}>
      <div className="flex-grow p-6">
            ...
            <Card className="bg-teal-900/50 border border-teal-500 backdrop-blur-sm overflow-hidden">
              <div className="p-6 flex flex-col h-full items-center">
                <div className="flex justify-between items-center w-full mb-4">
                  <h2 className="text-2xl font-bold text-blue-300">Level 60 Daily Loot Box</h2>
        ...
        <Card className="bg-teal-900/50 border border-teal-500 backdrop-blur-sm p-4 mb-6">
          <h3 className="text-xl font-bold text-blue-300 mb-4 text-center">Level 60 Daily Loot Box Rewards</h3>
        ...
        <Card className="w-full bg-teal-900/50 border border-teal-500 backdrop-blur-sm p-4 flex flex-col items-center text-center">
          ...
          <p className="text-2xl font-extrabold text-yellow-400 mb-4">
            Spin and win amazing rewards – common prizes of 0.6 KAS or an Ultra Rare prize of 1400 KAS!
          </p>
        </Card>
      </div>
      <SiteFooter />
...
function DailyLootBoxGame({ isPlaying, onGameEnd }: { isPlaying: boolean; onGameEnd: (item: any) => void; }) {
  ...
  useEffect(() => {
    if (isPlaying && !spinTriggered.current) {
      ...
      let winItem;
      if (r < 0.9999) {
        // 99.99% chance to win a common reward (0.6 KAS)
        ...
        // Ultra Rare reward: 1400 KAS win
