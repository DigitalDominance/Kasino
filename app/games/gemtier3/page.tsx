...
// =============================================================================
// GEM CRATE TIER 3 GAME COMPONENT & CONTENT
// =============================================================================

const tier3CrateItems = [
  { id: 1, name: "Lucky Gem", tier: "gem-common", reward: 5, image: "/GemCommon1.webp" },
  { id: 2, name: "Shiny Stone", tier: "gem-common", reward: 5, image: "/GemCommon2.webp" },
  { id: 3, name: "Sparkling Rock", tier: "gem-common", reward: 5, image: "/GemCommon3.webp" },
  { id: 4, name: "Golden Nugget", tier: "gem-common", reward: 5, image: "/GemCommon4.webp" },
  // Last 3 items become "rare" with a 75 KAS reward
  { id: 5, name: "Mystic Orb", tier: "gem-rare", reward: 75, image: "/GemCommon5.webp" },
  { id: 6, name: "Enchanted Crystal", tier: "gem-rare", reward: 75, image: "/GemCommon6.webp" },
  { id: 7, name: "Ruby Shard", tier: "gem-rare", reward: 75, image: "/GemCommon7.webp" },
  { id: 8, name: "Diamond Prize", tier: "gem-ultra-rare", reward: 250, image: "/GemUltraRare.webp" },
];
...
function getGemRarityStyle(tier: string) {
  switch (tier) {
    ...
    case "gem-rare":
      return "border-blue-300 bg-blue-900/30";
    case "gem-ultra-rare":
...
function getGemRarityOverlayClass(tier: string) {
  switch (tier) {
    ...
    case "gem-rare":
      return "bg-gradient-to-br from-blue-300/30 to-blue-900/30";
    case "gem-ultra-rare":
...
// Main Page Component for Gem Crate Tier 3 Game
export default function GemCrateTier3GamePage() {
  return <GemCrateTier3Content />;
}

function GemCrateTier3Content() {
  ...
  // Start the Tier 3 crate game on the backend using the gemcrate API
  const handleOpenCrate = async () => {
    ...
    try {
      ...
      const walletAddress = accounts[0];
      // Pass both the walletAddress and the gems being spent (1000 for Tier 3)
    ...
    <div className={`${montserrat.className} min-h-screen bg-black text-white flex flex-col`}>
      <div className="flex-grow p-6">
            ...
            <Card className="bg-green-900/50 border border-green-500 backdrop-blur-sm overflow-hidden">
              <div className="p-6 flex flex-col h-full items-center">
                <div className="flex justify-between items-center w-full mb-4">
                  <h2 className="text-2xl font-bold text-green-300">Gem Crate Tier 3</h2>
        ...
        <Card className="bg-green-900/50 border border-green-500 backdrop-blur-sm p-4 mb-6">
          <h3 className="text-xl font-bold text-green-300 mb-4 text-center">Gem Crate Tier 3 Rewards</h3>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
            {tier3CrateItems.map((item) => {
              const rarityClass = getGemRarityStyle(item.tier);
              const displayTier =
                item.tier === "gem-ultra-rare"
                  ? "Ultra Rare"
                  : item.tier === "gem-rare"
                  ? "Rare"
                  : item.tier.replace("gem-", "");
              return (
        ...
        <Card className="w-full bg-green-900/50 border border-green-500 backdrop-blur-sm p-4 flex flex-col items-center text-center">
          ...
          <p className="text-2xl font-extrabold text-green-400 mb-4">
            Open the crate for a chance to win common rewards, rare prizes, or the ultra-rare diamond!
          </p>
          <p className="text-lg text-green-200">Cost per play: 1000 Gems</p>
        </Card>
      </div>
      <SiteFooter />
      <AnimatePresence>
...
function GemCrateGame({ isPlaying, onGameEnd }: { isPlaying: boolean; onGameEnd: (item: any) => void; }) {
  ...
  useEffect(() => {
    if (isPlaying && !spinTriggered.current) {
      ...
      // Generate a random reel of 40 items from tier3CrateItems
      const randomReel = Array.from({ length: 40 }, () => tier3CrateItems[Math.floor(Math.random() * tier3CrateItems.length)]);
      ...
      // Ultra rare chance: 0.01%, then rare: 12.5%, otherwise common.
      const r = Math.random();
      let winItem;
      if (r < 0.0001) {
        winItem = tier3CrateItems.find(item => item.tier === "gem-ultra-rare");
      } else if (r < 0.125) {
        const rareRewards = tier3CrateItems.filter(item => item.tier === "gem-rare");
        winItem = rareRewards[Math.floor(Math.random() * rareRewards.length)];
      } else {
        const commonRewards = tier3CrateItems.filter(item => item.tier === "gem-common");
            ...
            <motion.div initial={{ scale: 0.8 }} animate={{ scale: [1, 1.4, 1] }} transition={{ times: [0, 0.5, 1], duration: 2, ease: "easeInOut" }} className="text-center p-6 rounded-lg border-2 border-green-400 shadow-[0_0_25px_8px_rgba(0,255,0,0.5)] bg-green-800/80 max-w-xs">
              ...
              <p className="text-xl font-bold text-green-100">
                ...
                <span className="text-base text-green-200">
                  {winningItem.tier === "gem-ultra-rare"
                    ? "Ultra Rare"
                    : winningItem.tier === "gem-rare"
                    ? "Rare"
                    : winningItem.tier.replace("gem-", "")}
                </span>
              </p>
