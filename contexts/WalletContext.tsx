...
export const WalletProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const handleBalanceChanged = async (balance: any) => {
    setBalance(Number(balance.balance.mature) / Math.pow(10, 8))
  }
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        className={`fixed bottom-4 left-4 p-4 rounded-md shadow-md ${
          type === "success"
            ? "bg-gradient-to-r from-[#49EACB] to-[#49EACB]/80 text-black"
            : "bg-gradient-to-r from-red-600 via-red-800 to-black text-white"
        } max-w-sm z-50`}
      >
        {message}
      </motion.div>
